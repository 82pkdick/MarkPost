...
export const getThumbnailPath = (post: LoadedPostData) => {
  ...
  if (postid && postid !== '' && thumb && thumb !== '') {
    ...
    thumbUrl = `${base}${config.ThumbnailSubImage}`;
  }
...
export const updateResourceData = async () => {
  ...
  const updateResourcesUrl = `${base}api/data-update`;
...
export const updateOnePostArchive = async () => {
  ...
  const updateArchiveUrl = `${base}api/archive/${sectionPath}/${postId}`;

  const updateAcvResponse = await fetch(updateArchiveUrl);
...
export const deleteOnePostArchive = async (pid: string) => {
  const deleteArchiveUrl = `${base}api/archive/notrequired/${pid}`;
