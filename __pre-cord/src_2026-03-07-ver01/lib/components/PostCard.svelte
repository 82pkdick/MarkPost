<script lang="ts">
  ...
  import { setPostPanelState, currentPostIns as currentPost } from "$lib/index.svelte";
  import { setCurrentPost } from "$lib/components/PostPanel.svelte";
  ...
  const handlePostState = async (event: MouseEvent, postid: string) => {
    ...
    const data = await currentPost.loadData(postid);
    if (data) {
      setCurrentPost(data);
    }
  };
