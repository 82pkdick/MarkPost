...
//-- MarkDownloaderコマンド
const markDownloader = pathSettings.MarkDownloaderCommandPath;
 ...
 * MarkDownloader の実行
 */
export const markDownload = async (srcUrl: string) => {
  let serverMsg = '';

  // console.log('@markDownload cmd 01 srcUrl: ', srcUrl);

  try {
    let command = `sleep 1; cd ${markDownloader};`;
    command += `./main-command.sh ${srcUrl};`;

    let stdout = await execCommand(command);

    console.log('command stdout: ', stdout.toString());
    serverMsg = '>>> Create new post data done.';
    console.log(serverMsg);
 ...
 * そこからHTMLにビルド。その後、`content-original/unsorted`に移動(npm run workpost-mvto-origin)。
