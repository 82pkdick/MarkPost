<script lang="ts">
  ...
  import { delTrSlash, getTimeStamp, deleteUrlParam } from "$lib/utils/locationUtils";
  ...
  const { contentsDir, searchq }: { contentsDir: string; searchq?: string | undefined; } = $props();

  let currentPost = $state(getCurrentPost());

  let editorMode = $derived(getEditorMode());

  const baseUrl = delTrSlash(resolve('/'));

  let panelUrl = $derived.by(() => {
    if (searchq && searchq !== '') {
      const fixedUrl = deleteUrlParam(page.url.href, 'q');
      return `${fixedUrl}?q=${searchq}`;
    } else {
  ...
  const setArchiveUrl = async () => {
    ...
    if (currentPost.available()) {
      ...
      archiveUrl = `${archiveUrlBase}/${postId}/index.html?v=${timestamp}`;
  ...
  $effect(() => {
    ...
    // console.log('A02 panelUrl: ', panelUrl);
  });
</script>
