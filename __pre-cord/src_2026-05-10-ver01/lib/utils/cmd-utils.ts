...
export const mediaDownload = async (postId: string) => {
  ...
  try {
    const result = await downloadPageMedia( postId );
    const serverMsg = result ? result.message : 'Failed to download resources for the new post.';

    if (result && result.ok) {

      //-- ここで記事のビルドはしない --//
      // buildArchive({ section: defaultSection, postId: postId });
