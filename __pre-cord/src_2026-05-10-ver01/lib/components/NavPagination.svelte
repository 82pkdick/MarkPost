<script lang="ts">
  ...
  import { closePostPanel, resetTagFiltering } from "$lib/index.svelte";
  ...
  const setPageInfos = () => {
    ...
    let basepath = `${baseUrl}/${pathSegment}`;

    prevUrl = PrevURL ? `${basepath}page/${(pageNumber - 1)}` : null;
    nextUrl = NextURL ? `${basepath}page/${(pageNumber + 1)}` : null;
    firstUrl = `${basepath}page/1`;
    lastUrl = `${basepath}page/${totalPages}`;
    currentPage = `${basepath}page/${pageNumber}`;
  };
  ...
  const setPageLinks = () => {
    ...
    let links = pageNumbers.map((number, idx) => {
      let basepath = `${baseUrl}/${pathSegment}`;
      link = { number: number, path: `${basepath}page/${number}` };
      return link;
    });
    pageLinks = links;
  };
