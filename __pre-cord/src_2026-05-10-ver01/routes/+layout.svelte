	...
	const getPageTitle = () => {
		...
		switch (true) {
			...
			case /\/sections\/([^\/]+)\/?$/.test(pathname):
				pagename = `section ${page.params.name}`;
				break;
			case /\/sections\/([^\/]+)\/(page)\/\d+\/?$/.test(pathname):
				pagename = `section ${page.params.name} page:${page.params.num}`;
				break;
			case /\/tags\/([^\/]+)\/?$/.test(pathname):
				pagename = `tags ${page.params.name}`;
				break;
			case /\/tags\/([^\/]+)\/(page)\/\d+\/?$/.test(pathname):
				pagename = `tags ${page.params.name} page:${page.params.num}`;
				break;
...
<svelte:head>
	...
	<link rel="stylesheet" href={`${baseUrl}/css/lib/destyle.css`} />
	<link rel="stylesheet" href={`${baseUrl}/css/lib/normalize.css`} />
	<link rel="stylesheet" href={`${baseUrl}/css/lib/syntax.css`} />
