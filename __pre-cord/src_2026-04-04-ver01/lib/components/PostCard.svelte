<script lang="ts">
  import type { LoadedPostData, SearchResultData } from "$lib/types";
  ...
  let { post, searchq = '', children }: { 
    post: LoadedPostData | SearchResultData; 
    searchq?: string; 
    children?: Snippet;
  } = $props();
  ...
  const handlePostState = async (event: MouseEvent, postId: string) => {
    let searchStr = (searchq && searchq !== '') ? searchq : undefined;
    ...
    // console.log('@postCard searchStr 01: ', searchStr);
    // console.log('@postCard searchStr 02: ', typeof searchStr);

    setPageParam(postId, searchStr);
  };
</script>
      ...
      <h3 class="resource-title ft-title">
        <div data-postid="{post.postId}">
          <button type="button" 
            ...
            onclick={(e) => handlePostState(e, post.postId)}
          >
