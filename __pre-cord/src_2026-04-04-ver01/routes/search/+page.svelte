<script lang="ts">
  ...
  import { getParams, deleteParams } from "$lib/utils/locationUtils";
  import { 
    getCurrentPost, 
    closePostPanel, 
  ...
  const StartSearch = async () => {
    ...
    //-- 見つかった記事のIDリスト
    let resultedPostIds: string[] | undefined = [];
    ...
    if (result.length > 0) {
      ...
      resultedPostIds = resData?.map((p) => p.postId);
  ...
  const handlePopState = async () => {
    ...
    console.log('@PopState 01 qstr: ', qstr);
    // console.log('@PopState 01 page: ', page);
    if (qstr && qstr !== "") {
      handleSetSearchQ(qstr);
      const postIds = await StartSearch();
      let qpost = getParams("post") || "";
      console.log('@PopState 02 qpost: ', qpost);
      if (postIds && postIds.length > 0) {
        if (!postIds.includes(qpost)) {
          console.log('@PopState 03 qpost: ', qpost);

          // const url = new URL(page.url.href);
          // url.searchParams.delete('post');
          // console.log('@PopState 04 qpost: ', url);

          // replaceUrl(`${url}`);

          // deleteParams('post');
          closePostPanel();
          replaceHistory(`search?q=${qstr}`);
        } else {
          // console.log('@PopState 04 qpost: ', qpost);
          openPanelFromParamId();
        }
      } else {
        closePostPanel();
      ...
      console.log('@PopState 05 page: ', page);
      closePostPanel();
      // replaceHistory('search');
      handleInitSearchQ();
    }
        ...
        {#if appliedPosts && appliedPosts.length > 0}
          {#each appliedPosts as post}
            ...
            {post.postId === currentPostId ? 'current' : ''}">
              <PostCard post={post} searchq={qword}>
