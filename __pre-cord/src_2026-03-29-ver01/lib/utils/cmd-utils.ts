 ...
 * MarkDownloader の実行
 */
export const markDownload = async (targetUrl: string) => {
  let serverMsg = '';

  try {
    let command = `sleep 1; cd ${markDownloader};`;
    command += `./main-command.sh ${targetUrl};`;
...
const createNewArchive = async () => {
  ...
  // console.log('createNewArchive 01 workDir: ', workDir);
  ...
  try {
    ...
    // console.log('createNewArchive 02 command: ', command);
