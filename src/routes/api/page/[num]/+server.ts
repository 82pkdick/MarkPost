...
export const GET: RequestHandler = async ({ params, request, fetch }) => {
  ...
  if (pageNumber === 1) {
    dataUrl = `${dataUrlBase}/index.json?v=${timestamp}`;    
  } else {
    dataUrl = `${dataUrlBase}/page/${pageNumber}/index.json?v=${timestamp}`;
  } 
  
  // console.log('API02 dataUrl: ', dataUrl);
