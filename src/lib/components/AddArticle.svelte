...
*/
const createNewArchive = async (postId: string) => {
  ...
  if (response.ok) {
    ...
    error(500, {
      message: 'Failed to create new post archive.'
    });
  }
};
    ...
    if (result.ok) {
      ...
      if (updateResourceResult.ok) {
        ...
        if (dPRresponse && dPRresponse.result) {
          ...
          const createNewArchiveResult = await createNewArchive(postId);
          if (createNewArchiveResult && createNewArchiveResult.result) {
            console.log(createNewArchiveResult.message);
          } else {
            error(500, {
              message: 'Failed to create new post archive.'
            });
