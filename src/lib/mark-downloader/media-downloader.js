...
const readDataFile = (postId, rootDir) => {
	const postFolder = `${rootDir}/${config.saveDir}/${postId}`;
...
const downloadPageMedia = async (postId, rootDir) => {
	const mediaData = readDataFile(postId, rootDir);
	if (!mediaData) {
		return;
	}
